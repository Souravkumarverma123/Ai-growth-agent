...
- 2026-09-05: FIXED — TICKET-302 added a unique constraint on `orders.offer_id`
  (already migrated) plus `reserveOrder` in
  `packages/database/repositories/orders.ts`, and wired it into
  `createOrder(offerId)` as a reserve-before-POST step. See the ISSUE-010
  entry's "Fix" section above — updated in place once TICKET-302 landed.
